...
{    
	vec3 pos = aPosition;
	Normal = normalize((model * vec4(aNormal,0.0f)).xyz); //works only for uniform scaling
	if(aNormal.y > 0.0f)
	{	
		...
		Normal = normalize((model * vec4(normalize(cross(n2, n1)), 0.0f)).xyz);
	}
	if(isnan(aPosition.y))
	{
	    pos.y = 0.0f;
		vec4 texel = textureLod(samp, aTexCoords, 0.0f);
		pos += vec3(0.0f, texel.r, 0.0f);
	}
